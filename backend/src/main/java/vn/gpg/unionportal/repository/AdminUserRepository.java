...
public interface AdminUserRepository extends JpaRepository<AdminUser, Long>, JpaSpecificationExecutor<AdminUser> {
    ...
    @Query("select account.activeTokenId from AdminUser account where lower(account.username) = lower(:username) and account.active = true")
    Optional<String> findActiveTokenId(@Param("username") String username);
