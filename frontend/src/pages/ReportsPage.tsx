...
import { api, apiAll, currentMonth, downloadFile, formatMoney } from '../api'
...
const MONTHLY_REPORT_REQUIREMENTS = [
  ...
  ['D. Kiến nghị NLĐ', 'Mã kiến nghị; nhóm vấn đề; PIC; deadline; trạng thái; kết quả'],
  ['E. Chương trình', 'Tên chương trình; số người; chi phí; tỷ lệ tham gia; báo cáo'],
  ['F. Thu – chi', 'Thu kỳ này; chi; số dư; chứng từ chưa hoàn thiện'],
  ['G. Kế hoạch tháng tới', 'Chương trình; thời gian; PIC; ngân sách dự kiến'],
...
export default function ReportsPage({ units, isAdmin, currentUnitId, currentUnitName, currentUserName }: Props) {
  ...
  const activityOutput = featuredActivity
    ...
    : 'Chưa có chương trình trong kỳ báo cáo'
  const monthlyReportOutputs = summary ? [
    ...
    `${summary.activeEmployees} NLĐ / ${summary.unionMembers} đoàn viên / ${summary.memberChanges} lần cập nhật / ${membershipRate}% tham gia`,
    ...
    `${summary.laborCases} kiến nghị: ${summary.closedLaborCases} đã đóng / ${openCases} đang xử lý`,
    activityOutput,
  ...
  const reportQuery = `month=${encodeURIComponent(month)}${unitId ? `&unitId=${encodeURIComponent(unitId)}` : ''}`
  const periodicReportExportPath = `/spreadsheets/reports/periodic.xlsx?${reportQuery}`
  const companySummaryExportPath = `/spreadsheets/reports/company-summary.xlsx?${reportQuery}`
  const reportExcelActions = <>
    <button className="button button--ghost" onClick={() => void downloadFile(companySummaryExportPath, `bao-cao-tong-hop-cong-doan-${month}.xlsx`)
      .catch(err => setError(err instanceof Error ? err.message : 'Không thể xuất báo cáo tổng hợp'))}>Xuất tổng hợp</button>
    <ExcelImportActions resource="reports" filename={`bao-cao-dinh-ky-${month}.xlsx`}
      downloadPath={periodicReportExportPath} importLabel="Nhập Excel" templateLabel="Xuất báo cáo định kỳ"
  ...
  if (isAdmin) return (
    <section className="page-section">
      <div className="page-heading">
        <div><h1>Báo cáo điều hành hệ sinh thái</h1></div>
      ...
      {summary && <div className="admin-report-grid">
        <article className="panel data-card admin-report-card">
          <div className="panel__heading"><div><h2>Xu hướng toàn Hệ sinh thái</h2></div></div>
          <dl className="summary-list"><div><dt>Đoàn viên / NLĐ</dt><dd>{summary.unionMembers} / {summary.activeEmployees}</dd></div><div><dt>Cập nhật thông tin đoàn viên</dt><dd>{summary.memberChanges} <small>{trendText(summary.memberChanges, previousSummary?.memberChanges)}</small></dd></div><div><dt>Kiến nghị phát sinh</dt><dd>{summary.laborCases} <small>{trendText(summary.laborCases, previousSummary?.laborCases)}</small></dd></div><div><dt>Lượt tham gia chương trình</dt><dd>{summary.participants} <small>{trendText(summary.participants, previousSummary?.participants)}</small></dd></div></dl>
        </article>

        <article className="panel data-card admin-report-card">
          <div className="panel__heading"><div><h2>Đơn vị có vấn đề lặp lại/quá hạn</h2></div></div>
        ...
        <article className="panel data-card admin-report-card">
          <div className="panel__heading"><div><h2>Hiệu quả chính sách/chương trình</h2></div></div>
          <dl className="summary-list"><div><dt>Chăm lo hoàn tất</dt><dd>{welfareCompletionRate}%</dd></div><div><dt>Kiến nghị đã đóng</dt><dd>{caseClosureRate}%</dd></div><div><dt>Chương trình thực hiện</dt><dd>{summary.activities}</dd></div><div><dt>Bình quân tham gia/chương trình</dt><dd>{averageParticipation}</dd></div></dl>
        </article>

        <article className="panel data-card admin-report-card">
          <div className="panel__heading"><div><h2>Tình hình ngân sách và chăm lo</h2></div></div>
        ...
        <article className="panel data-card admin-report-card admin-report-card--risk">
          <div className="panel__heading"><div><h2>Rủi ro cần quyết định</h2></div></div>
          <dl className="summary-list"><div><dt>Kiến nghị quá hạn</dt><dd>{overdueCases.length}</dd></div><div><dt>Kiến nghị đang mở</dt><dd>{openCases}</dd></div><div><dt>Chứng từ chưa đủ</dt><dd>{summary.incompleteDocuments}</dd></div><div><dt>CĐCS chưa nộp báo cáo</dt><dd>{missingCount}</dd></div></dl>
        </article>

        <article className="panel data-card admin-report-card">
          <div className="panel__heading"><div><h2>Đề xuất chính sách/nguồn lực</h2></div></div>
    ...
    <section className="page-section">
      ...
      {summary && <div className="admin-report-grid">
        ...
        <article className="panel data-card admin-report-card admin-report-card--risk">
          ...
          <dl className="summary-list"><div><dt>Kiến nghị quá hạn</dt><dd>{overdueCases.length}</dd></div><div><dt>Kiến nghị đang mở</dt><dd>{openCases}</dd></div><div><dt>Chứng từ chưa đủ</dt><dd>{summary.incompleteDocuments}</dd></div><div><dt>Chăm lo chưa hoàn tất</dt><dd>{unfinishedWelfare}</dd></div></dl>
        </article>
