...
export const welfareFields: FieldConfig[] = [
  ...
  { name: 'eventDate', label: 'Ngày phát hiện', type: 'date', required: true }, { name: 'deadline', label: 'Hạn hoàn tất (tự tính)', type: 'date', required: true, readOnly: true },
  ...
  { name: 'amount', label: 'Số tiền', type: 'number', step: '1000', required: true, defaultValue: '0', currency: true },
  { name: 'standardAmount', label: 'Định mức theo chính sách', type: 'number', step: '1000', readOnly: true, currency: true },
...
export const welfarePolicyFields: FieldConfig[] = [
  ...
  { name: 'supportAmount', label: 'Mức hỗ trợ (VNĐ)', type: 'number', min: '0', step: '1000', required: true, defaultValue: '0', currency: true },
...
export const caseFields: FieldConfig[] = [
  { name: 'caseCode', label: 'Mã kiến nghị', required: true }, { name: 'receivedDate', label: 'Ngày tiếp nhận', type: 'date', required: true },
  { name: 'unionUnitId', label: 'Đơn vị', type: 'unit', required: true }, { name: 'requesterName', label: 'Người được hỗ trợ', required: true },
  { name: 'employeeCode', label: 'Mã NV người được hỗ trợ' }, { name: 'jobTitle', label: 'Chức danh' },
  ...
  { name: 'source', label: 'Kênh tiếp nhận' }, { name: 'issueGroup', label: 'Nhóm vấn đề', type: 'select', required: true, options: [] },
  ...
  { name: 'attachmentFile', label: 'Tệp đính kèm', type: 'file', wide: true },
  { name: 'attachmentNote', label: 'Ghi chú / liên kết tài liệu', type: 'textarea', wide: true },
  { name: 'resultText', label: 'Kết quả xử lý', type: 'textarea', wide: true },
...
export const caseIssueGroupFields: FieldConfig[] = [
  { name: 'code', label: 'Mã nhóm', required: true, placeholder: 'VD: LUONG_THUONG' },
  { name: 'name', label: 'Tên nhóm vấn đề', required: true },
  { name: 'active', label: 'Đang áp dụng', type: 'checkbox', defaultValue: true },
]

export const activityFields: FieldConfig[] = [
  { name: 'activityCode', label: 'Mã chương trình', required: true, section: 'Thông tin kế hoạch', sectionDescription: 'Chỉ nhập thông tin cần thiết để khởi tạo chương trình' },
  ...
  { name: 'plannedBudget', label: 'Ngân sách dự kiến', type: 'number', step: '1000', required: true, defaultValue: '0', currency: true },
  ...
  { name: 'quickFeedback', label: 'Báo cáo', type: 'textarea', hidden: true },
  ...
  { name: 'actualCost', label: 'Chi phí thực tế', type: 'number', defaultValue: '0', hidden: true, currency: true },
...
export const financeFields: FieldConfig[] = [
  ...
  { name: 'category', label: 'Nhóm nghiệp vụ', required: true }, { name: 'amount', label: 'Số tiền', type: 'number', step: '1000', required: true, currency: true },
